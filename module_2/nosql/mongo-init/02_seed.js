...
const teachersCount = Number(process.env.TEACHERS_COUNT || 5);
const studentsCount = Number(process.env.STUDENTS_COUNT || 50);
const coursesCount = Number(process.env.COURSES_COUNT || 8);
const gradesPerStudentMin = Number(process.env.GRADES_PER_STUDENT_MIN || 2);
const gradesPerStudentMax = Number(process.env.GRADES_PER_STUDENT_MAX || 4);

if (gradesPerStudentMin > gradesPerStudentMax) {
  throw new Error('GRADES_PER_STUDENT_MIN must be <= GRADES_PER_STUDENT_MAX');
}
...
for (let s = 0; s < students.length; s++) {
  ...
  const cCount = randomInt(gradesPerStudentMin, gradesPerStudentMax);
  const used = new Set();
